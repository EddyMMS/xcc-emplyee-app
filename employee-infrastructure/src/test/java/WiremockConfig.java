...
public class WiremockConfig {
  ...
  void stubRandomEmployee() throws JsonProcessingException {
    var responseBody =
        new RandomEmployeeResultsDTO()
            .results(
                List.of(
                    new RandomEmployeeEmployeeDTO()
                        .gender("male")
                        .name(new RandomEmployeeNameDTO().first("Hans").last("Tolonen"))));

    stubRandomEmployee(responseBody);
  }

  void stubRandomEmployee(RandomEmployeeResultsDTO randomEmployeeResultsDTO)
      throws JsonProcessingException {
    stubFor(
        get("/random-employee/api/")
            .willReturn(
                ok(objectMapper.writeValueAsString(randomEmployeeResultsDTO))
